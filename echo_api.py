...
def prepare_shared_context(data, source_override=None):
    ...
    # ── MEMORY SUMMARY: BASIC+ SEULEMENT ──────────────────────────────────────
...
# ── TIMEOUT PAR THREADING (fonctionne partout, pas juste Linux) ───────────────
...
# steps = [(client, model_key, timeout_sec), ...]
...
# ── /home : gemini_free_2 2s → deepseek 2s → compound 2s → glm 3s → paid 25s ─
...
# ── /chat : gemini_free_1 2s → kimi 2s → glm 3s → compound 2s → compound 2s → paid 25s ─
...
# ── /vitality : glm 4s → compound 4s → paid 25s ──────────────────────────────
...
# ── /history : compound 4s → nemotron 4s → paid 25s ──────────────────────────
...
# ── /books : kimi 2s → glm 3s → compound 2s → nemotron 2s → deepseek 2s → paid 25s ─
...
# ── /horizon : nemotron 3s → gemini_free_2 3s → gemini_free_1 3s → glm 3s → deepseek 3s → paid 25s ─
...
def memory_summary():
    try:
        ...
        recent_text = "\n".join(messages[-100:])
        prompt = (
            "Tu es un moteur de memoire long terme.\n\n"
            f"Resume existant :\n{old_summary}\n\n"
            f"Nouveaux messages :\n{recent_text}\n\n"
            "Mets a jour la memoire. Conserve : identite utilisateur, preferences, projets, "
            "livres, habitudes, abonnements, gouts, informations durables. "
            "Ignore : bavardage temporaire, salutations, petites discussions sans valeur. "
            "Reponds uniquement avec le resume mis a jour."
        )
